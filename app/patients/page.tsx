		...
		<div className="space-y-6">
			<div className="flex justify-between items-center">
				<h1 className="text-3xl font-bold">
					Billing and Appointments for {selectedPatient.name}
				</h1>
				<AlertDialog>
					...
					<AlertDialogContent>
						<AlertDialogHeader>
							...
							<AlertDialogDescription>
								...
								patient and all associated data including appointments, billing
			...
			<Card>
				...
				<CardContent>
						...
						{Object.entries(patientInfo).map(([key, value]) => (
							...
							>
								...
								<Input
									...
									value={value}
									onChange={(e) =>
										handleInputChange('patientInfo', key, e.target.value)
									}
					...
					{editingPatientInfo && (
						<Button
							className="mt-4"
							onClick={() => handleSave('patientInfo')}
						>
							Save Patient Info
						</Button>
					)}
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex justify-between items-center">
						...
						<Button
							onClick={() =>
								setEditingResponsibleParty(!editingResponsibleParty)
							}
						>
				...
				<CardContent>
					<div className="grid grid-cols-2 gap-4">
						{Object.entries(responsibleParty).map(([key, value]) => (
							<div
								key={key}
								className="space-y-2"
							>
								...
								{key === 'insuranceType' ? (
									<Select
										...
										onValueChange={(newValue) =>
											handleInputChange('responsibleParty', key, newValue)
										}
									...
									<Input
										...
										value={value}
										onChange={(e) =>
											handleInputChange('responsibleParty', key, e.target.value)
										}
										disabled={!editingResponsibleParty}
									/>
								)}
					...
					{editingResponsibleParty && (
						<Button
							className="mt-4"
							onClick={() => handleSave('responsibleParty')}
						>
							Save Responsible Party Info
						</Button>
					)}
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex justify-between items-center">
						...
						<Button
							onClick={() => setEditingInsuranceInfo(!editingInsuranceInfo)}
						>
				...
				<CardContent>
							...
							>
								...
								{key === 'subscriberRelationship' ? (
									<Select
										value={value}
										onValueChange={(newValue) =>
											handleInputChange('insuranceInfo', key, newValue)
										}
									...
									<Input
										...
										value={value}
										onChange={(e) =>
											handleInputChange('insuranceInfo', key, e.target.value)
										}
										disabled={!editingInsuranceInfo}
									/>
								)}
					...
					{editingInsuranceInfo && (
						<Button
							className="mt-4"
							onClick={() => handleSave('insuranceInfo')}
						>
							Save Insurance Info
						</Button>
					)}
